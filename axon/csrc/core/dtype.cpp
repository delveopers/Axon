...
int64_t clamp_to_int_range(double value, dtype_t dtype) {
  switch (dtype) {
    ...
    case DTYPE_INT64:
      ...
      return (int64_t)round(value);
    default: return (int64_t)round(value);
  }
}
...
float* convert_to_float32(void* data, dtype_t dtype, size_t size) {
  ...
  for (size_t i = 0; i < size; i++) float_data[i] = dtype_to_float32(data, dtype, i);
  return float_data;
}

void convert_from_float32(float* float_data, void* output_data, dtype_t dtype, size_t size) {
  for (size_t i = 0; i < size; i++) float32_to_dtype(float_data[i], output_data, dtype, i);
}
...
dtype_t promote_dtypes(dtype_t dtype1, dtype_t dtype2) {
  if (dtype1 == dtype2) return dtype1;  // ff same dtype, return it
  if (is_float_dtype(dtype1) && is_integer_dtype(dtype2)) return dtype1;  // float types always win over integer types
  if (is_float_dtype(dtype2) && is_integer_dtype(dtype1)) return dtype2;
  if (is_float_dtype(dtype1) && is_float_dtype(dtype2)) return (get_dtype_size(dtype1) >= get_dtype_size(dtype2)) ? dtype1 : dtype2;    // if both are float types, choose the larger one
  ...
  if (is_integer_dtype(dtype1) && is_integer_dtype(dtype2)) {
    ...
    if (is_signed_dtype(dtype1) != is_signed_dtype(dtype2)) {
      ...
      if (get_dtype_size(unsigned_type) >= get_dtype_size(signed_type)) { return unsigned_type; }
      else {
    ...
    return (get_dtype_size(dtype1) >= get_dtype_size(dtype2)) ? dtype1 : dtype2;  // both have same signedness, choose larger size
  }
