...
Array* linear_1d_array(Array* weights, Array* input, Array* bias) {
  if (weights == NULL || input == NULL) {
    fprintf(stderr, "Weight and input arrays cannot be null!\n");
    exit(EXIT_FAILURE);
  }
  ...
  // Convert arrays to float32 for computation
  ...
  for (int i = 0; i < weights->shape[0]; i++) { dot_array_ops(w_float + i * weights->shape[1], x_reshaped, out + i, weights->shape[1]); }
  free(x_reshaped);
...
Array* linear_2d_array(Array* weights, Array* input, Array* bias) {
  if (weights == NULL || input == NULL) {
    fprintf(stderr, "Weight and input arrays cannot be null!\n");
    exit(EXIT_FAILURE);
  }
  ...
  transpose_2d_array_ops(w_float, w_transposed, weight_shape);
  
  int input_shape[2] = {input->shape[0], input->shape[1]};
  int transposed_weight_shape[2] = {weights->shape[1], weights->shape[0]};
  matmul_array_ops(x_float, w_transposed, out, input_shape, transposed_weight_shape);
...
Array* linear_transform_array(Array* weights, Array* input, Array* bias) {
  if (input == NULL || weights == NULL) {
    fprintf(stderr, "Input and weight arrays cannot be null!\n");
    exit(EXIT_FAILURE);
  }

  if (input->ndim == 1 || weights->ndim == 1) { return linear_1d_array(weights, input, bias); }
  else if (input->ndim == 2 || weights->ndim == 2) { return linear_2d_array(weights, input, bias); }
  else {
    fprintf(stderr, "Linear transformation only supports 1D or 2D input arrays\n");
    exit(EXIT_FAILURE);
  }
}