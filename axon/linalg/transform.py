...
def linear(a: array, b: array, bias: array = None, dtype: DType = 'float32') -> array:
  a, b = a if isinstance(a, array) else array(a, 'float32'), b if isinstance(b, array) else array(b, 'float32')
  if bias is not None:
    ptr = lib.linear_transform_array(a.data, b.data, bias.data if isinstance(bias, array) else bias).contents
  else:
    if a.ndim == 1: ptr = lib.linear_1d_array(a.data, b.data, None).contents
    elif a.ndim == 2: ptr = lib.linear_2d_array(a.data, b.data, None).contents
    else: ptr = lib.linear_transform_array(a.data, b.data, None).contents

  out = array(ptr, dtype if dtype is not None else a.dtype or b.dtype)
