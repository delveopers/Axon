...
class ShapeHelp:
  ...
  def transpose_shape(shape: tuple) -> list: return shape if len(shape) == 1 else [shape[1], shape[0]] if len(shape) == 2 else [shape[0], shape[2], shape[1]] if len(shape) == 3 else (_ for _ in ()).throw(ValueError(f"Unsupported shape dimension: {len(shape)}"))
